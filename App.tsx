...
const App: React.FC = () => {
  ...
  console.log("App Component Rendering, Current User:", currentUser?.name || "None");
      ...
      <header className="sticky top-0 z-40 premium-blur px-6 py-4 flex justify-between items-center border-b border-white/5">
            ...
            {syncError && (
              ...
              >
                <span className="text-[7px] font-black text-amber-500 uppercase tracking-widest leading-none">
                  {syncError.toLowerCase().includes("configured") ? "Config Error" : "Offline"}
                </span>
              </button>
            )}
          </div>
        ...
        <div className="flex items-center gap-4">
           ...
           <div className="w-8 h-8 rounded-full border border-white/10 overflow-hidden">
              <img src={currentUser?.avatar || 'https://picsum.photos/seed/default/200/200'} className="w-full h-full object-cover" />
           </div>
        </div>
      </header>
